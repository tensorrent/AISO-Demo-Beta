...
> **⚠️ Demonstration build — obfuscated.** This build reproduces the engine's **observable
> behaviour** only (four-verdict Council, integer token charges, symbolic pointers, constraint
> dispositions, the rolling scroll root). The **proprietary algorithms are not present** — SPE
> interning, BRA charge algebra, CDCL reduction, and manifold placement are replaced by
> deterministic, behaviour-faithful stand-ins, and the demo source is additionally obfuscated.
> Safe to share: no real IP is in this build. Deterministic by design — the same query always
> yields the same verdict, charge, and pointer.
...
- **A contradiction** → `quantum energy fields but not really a wave` → **CHALLENGE**.
